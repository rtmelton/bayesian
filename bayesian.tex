...
If the joint
$\measonprod\abscont (\measmu\times\marginalthree)^*$, with
$\funck$
denoting the corresponding Markov kernel given
$(\uspace,\sigalg{H})$ and $(\measurespace,\sigmaalgebra)$,
then for $\measlambda^*$\hyp{}almost all $\uspaceelt\in\uspace$, the measure
$\funck_\uspaceelt$
on $\sigmaalgebra$,
...
\]
We will show that for $\seti\in\sigmaalgebra$,
\[
\funck_\uspaceelt(\seti)
=
\int_\seti
...
\,d\measmu(\mspaceelt),
\]
for $\measlambda^*$\hyp{}almost all $\uspaceelt\in\uspace$.
...
Then
$\charfunction{\seti\times\setiii}$
is integrable with respect to the joint
$\measonprod$.
Since
$\measonprod \abscont (\measmu\times\marginalthree)^*$,
...
for $\marginalthree^*$\hyp{}almost all $\mspaceeltiii\in\measurespaceiii$, and where
$\funch(\uspaceelt)=\mspaceeltiii$.
By Proposition~\ref{preimage_of_a_null_set}, the preimage under $\funch$ of a
$\marginalthree^\kernast$\hyp{}null set of $\measurespaceiii$ is a
$\measlambda^\kernast$\hyp{}null set of $\uspace$.
It means that we have shown, making the substitution of $\funch(u)$ for $\mspaceeltiii$, that
\[
\funck_\uspaceelt(\seti)
=
\int_{\seti}
\frac{d\measonprod}{d(\measure\times\marginalthree)^*}
\bigl(\mspaceelt,\funch(\uspaceelt)\bigr)
d \measmu(\mspaceelt)
\]
for $\measlambda^\kernast$\hyp{}almost all $\uspaceelt\in\uspace$, as required.
\end{proof}
