...
Let $(\measurespace,\sigmaalgebra, \measmu)$ be a measure space.
Let us say that a
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}simple
function is a
$(\measurable{\measmu^\kernast},\borel_\R)$\hyp{}\term{step function} if it can be expressed as a linear combination of characteristic functions of subsets of
$\measurable{\measmu^\kernast}$ of finite
$\measmu^\kernast$\hyp{}measure.  That is,  if $\phi$ is a
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}simple function
having range
$\{a_1, \ldots, a_n\}$, then
$\phi$ is a $(\measurable{\measmu^\kernast},\borel_\R)$\hyp{}\term{step function} if
$\phi=\sum_{i=1}^n a_i\charfunction{\seti_i}$ with each
$\seti_i \in \measurable{\measmu^\kernast}$, and with $\measmu^*(A_i) < \infty$,
