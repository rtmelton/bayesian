...
Why change structure from measure spaces
...
One reason might be the use of \emph{density} functions, which represent a Radon-Nikodym derivative.
...
A consequence of this change of structure is that the measure $\measmu\times\measnu$ on the semiring
$\productsemiring{\sigmaalgebra}{\sigmaalgebraii}$ need no longer be a measure on
$\productsig{\sigmaalgebra}{\sigmaalgebraii}$, so we will use its unique extension
$(\measmu\times\measnu)^*$.
\end{rmk}
