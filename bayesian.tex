...
Will come back and transcribe my hand\hyp{}written stuff. At some point, maybe.
\end{proof}
...
What does it mean to show that
\[
...
By hypothesis,
$\funcf\in
L_1(\measurespace, \sigmaalgebra,\measmu)$ and
$\funcf$ is
...
$\phi_n(x) \uparrow f(x)$ for all $x\in\measurespace$, where each $\phi_n$ is a sum of the form
\[
\sum_{i=1}^m c_i \charfunction{\setiii_i}
\]
with each $\setiii_i \in \sigmaalgebraii$.


We can suppose that both $\funcf$ and $\funcg$ are nonnegative.
It means we can further suppose that the $\phi_n$ are also nonnegative.
