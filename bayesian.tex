...
Let $(\measurespace, \sigmaalgebra, \measmu)$ be a finite measure space, where $\sigmaalgebra$ is a $\textsigma$\hyp{}algebra, and let $\sigmaalgebraii$ be a $\textsigma$\hyp{}subalgebra of $\sigmaalgebra$.
If $\ellclass{f}{\measmu}
\in \cali{L}_1(\measurespace, \sigmaalgebra, \measmu)$, and if $f$ is
...
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable. We can suppose that $f\geq 0$.  By Proposition~\ref{measurable_implies_simple_function_limit}, there is a sequence $\{\phi_n\}$ of $(\sigmaalgebraii, \borel_\R)$\hyp{}simple functions such that
...
\]
with each $\setii_i \in \sigmaalgebraii$.
By hypothesis,  $(\measurespace, \sigmaalgebra, \measmu)$ is a finite measure space
finite measure space, and this means that each
...
$\setii\in\measurable{\measmu^\kernast}$, and so each $\phi_n$ is a
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}step function,
as required.
\end{proof}
