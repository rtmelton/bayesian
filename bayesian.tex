...
See that if more than one measure is involved, then we would have to specify.
Is it clear that with a finite measure space, every simple function is a step function?
\end{defn}
...
Let $(\measurespace, \sigmaalgebra, \measmu)$ be a finite measure space, where $\sigmaalgebra$ is a $\textsigma$-algebra, and let $\sigmaalgebraii$ be a $\textsigma$-subalgebra of $\sigmaalgebra$.
...
$\{ \phi_n \}$ of
$(\sigmaalgebraii, \borel_\R)$\hyp{}step functions with
...
So by Definition~\ref{step_function}, every
$\phi_n$ is a sum of the form
\[
\sum_{i=1}^m b_i \charfunction{\setii_i}
\]
with each $\setii_i \in \sigmaalgebraii$, as required.
\end{proof}
