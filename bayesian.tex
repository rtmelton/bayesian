...
The remaining propositions in this chapter are all stated in terms of \textbf{joint}, \textbf{marginal}, and \textbf{reference} measures.
\end{defn}
...
is regular, with
$\funck$
denoting the corresponding Markov kernel given
$(\uspace,\sigalg{H})$ and $(\measurespace,\sigmaalgebra)$,
and if
\[
\funcj \in
L_1\bigl(\measurespace\times\measurespaceiii,
\productsig{\sigmaalgebra}{\sigmaalgebraiii},
\measureiii \circ \preimage{(\function,\functioniii)} \bigr),
\]
then 
\[
\int_{\measurespace\times\measurespaceiii} \funcj \,
d \bigl( \measureiii \circ \preimage{(\function,\functioniii)} \bigr)
=
\int_\measurespaceiii\int_\measurespace
\funcj(\mspaceelt,\mspaceeltiii) \,
d \funck_\uspaceelt(\mspaceelt)\,
d \bigl( \measureiii \circ \preimage{\functioniii} \bigr)(\mspaceeltiii),
\]
where $\uspaceelt$ is such that $\funch(\uspaceelt) = \mspaceeltiii$.
\end{propboldnote}
...
is constant and is equal to $\funca(\mspaceeltiii)$, as was shown in Proposition~\ref{constant_on_preimage}.
And by the definition of the Markov kernel itself:
\[
\funck_\uspaceelt (\seti)
= \funck(\uspaceelt,\seti)
= \funck^\seti(\uspaceelt).
\]
...
Let us show that the required equality  holds for the function
$\funcj = \charfunction{\seti\times\setiii}$,
...
& = \inducedint_\uspace
\condexpsub{\bigl(
\ellclass{\charfunction{\preimage{\function}(\seti)}}{}
\cdot
\ellclass{\charfunction{\preimage{\functioniii}(\setiii)}}{}
\bigr)}
{\sigalg{H}}
\, d\measureiii
& \text{Proposition~\ref{equality_of_expected_values}}
\\
& = \inducedint_\uspace
\ellclass{\charfunction{\preimage{\functioniii}(\setiii)}}{}
\cdot
...
{\sigalg{H}}
\, d\measureiii
& \text{Proposition~\ref{measurable_functions_factor_out}}
\\
& = \int_\uspace
\charfunction{\preimage{\functioniii}(\setiii)}\cdot
(\funca \circ\functioniii)
\, d\measureiii
& \text{$\condexpsub{\bigl(
\ellclass{\charfunction{\preimage{\function}(\seti)}}{}
\bigr)}
{\sigalg{H}}
=
\ellclass{\funca \circ\functioniii}{}$}
\\
& = \int_\uspace
\bigl( (\charfunction{\setiii}\cdot \funca)\circ \functioniii \bigr)
\, d\measureiii
& \text{$\charfunction{\preimage{\functioniii}(\setiii)} = \charfunction{\setiii}\circ \functioniii$}
\\
& = \int_\measurespaceiii
(\charfunction{\setiii}\cdot \funca )
\, d\bigl(\measureiii \circ \preimage{\functioniii} \bigr)
& \text{change of variable}
\\
& = \int_\measurespaceiii
\charfunction{\setiii}(\mspaceeltiii)\cdot 
\funca(\mspaceeltiii)
\, d\bigl( \measureiii \circ\preimage{\funch} \bigr)(\mspaceeltiii)
&
\makebox[0pt][r]{$\funca(\mspaceeltiii) = \funck_\uspaceelt(\seti)$ if $\funch(\uspaceelt) = \mspaceeltiii$}
\\
& = \int_\measurespaceiii
...
Set the joint $\measonprod = \measureiii\circ\preimage{(\function,\functioniii)}$,
and set the marginal $\marginalthree = \measlambda\circ\preimage{\funch}$.
If\, $\measonprod \abscont
(\measure\times\measureiv)^*$ on
$\productsig{\sigmaalgebra}{\sigmaalgebraiii}$,
then the measure
$\funck_\uspaceelt$
...
\dfrac{d \marginalthree}{d \measrho}(\mspaceeltiii)
}.
\]
\end{propboldnote}
