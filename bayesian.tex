...
%
%For drop caps at beginnings of chapters. Fun, but questionable.
\usepackage{lettrine}
%
...
%where prefix is used to order the symbol in the Nomeclature list.
%
%
...
%[Edit] It's now REAL clear. NOT a good idea. Just don't.
%
%
\usepackage{mathrsfs}
%\usepackage{yfonts}
...
We will not expect the reader to ``keep this, that, and the other thing in mind.'' We spell it out instead.
...
Statistical models only started making sense after learning Bayesian statistical models, so that is all you will find here. All of the material regarding frequentist statistics has been edited out, like when a cocoon is left behind.

The notes that follow  might be described as more of a measure-theoretic companion to something  comprehensive and explanatory on Bayesian statistical models.  I strongly recommend Richard McElreath's \textsl{Statistical Rethinking}~\cite{sr_mcelreath_2020}.
...
The measure $\measure\circ\preimage{\function}$ on the \textsigma-algebra of the codomain of $\function$ in Proposition~\ref{function_induces_measure} is the \textbf{measure induced by $\function$}.\index{measure induced by a function}  Expect a result this fundamental to get used without much fanfare. The notation 
$\measure_\function$\nomenclature[mu sub f]{$\measure_\function$}{same as $\measure\circ\preimage{\function}$}
...
The quotient space $L_p(\measurespace, \semiring, \measure)$ need not form an algebra with respect to pointwise multiplication. Still, for classes $\ellclass{f}{\measure}$ and $\ellclass{g}{\measure}$ in $L_p(\measurespace, \semiring, \measure)$, if the function $\abs{fg}^p$ is integrable, then we can define the product 
...
\quad\text{for all $A\in \sigmaalgebra$.}
\]
Furthermore, we may suppose the representative function $f$ to be 
...
The idea of a product semiring generalizes in a clear way to the product of more than two factors if we make an identification.  That is, the Cartesian product is not associative so, for example, we treat
...
which contains the product semiring $\productsemiring{\sigmaalgebra}{\sigmaalgebraii}$.
...
\measonprod(\seti\times\measurespaceii)
=
\int_{\seti\times\measurespaceii} \function \, d(\prior\times\measureii).
\]
By Fubini's,
\[
\int_{\seti\times\measurespaceii} \function 
\, d(\prior\times\measureii)
...
Still, there are some authors that will confuse them.
\end{rmk}
...
If
$\funch_1,\funch_2: (\measurespace,\sigmaalgebra) \to (\measurespaceii,\sigmaalgebraii)$ 
are independent functions, and 
...
\begin{defnboldnote}[Conditional Mathematical Expectation]
...
The \emph{conditional mathematical expectation
$\condexpsub{{\lebclass{\funcf}}}{\sigmaalgebraii}$
of the class $\lebclass{\funcf}$ with respect to the
\textsigma-algebra}
...
$\funcg \in\cali{L}_1(\measurespace, \sigmaalgebra,\measure)$
satisfies the equation
%
\begin{equation}\label{cme_functional_equation}
...
\index{conditional expectation with respect to a sigma algebra@conditional expectation with respect to a 
$\textsigma$-algebra}
Let $(\measurespace, \sigmaalgebra,\measure)$ be a probability space, and let
$\ellclass{\function}{\measure}\in L_1(\measurespace, \sigmaalgebra,\measure)$.
...
\emph{conditional mathematical expectation of} $\ellclass{\function}{\measure}$ \emph{with respect to the} $\textsigma$-algebra $\sigmaalgebraii$, or simply the \emph{conditional expectation} of 
$\ellclass{\function}{\measure}$ with respect to $\sigmaalgebraii$. This defines an operator:
\[
...
Let $\sigmaalgebraii$ be a \textsigma-subalgebra of $\sigmaalgebra$. The conditional mathematical expectation defines a contractive operator:
\[
\kondexpop\nolimits_{\sigmaalgebraii}
:
L_1(\measurespace, \sigmaalgebra,\measure) \to
...
By definition, 
$\expval{\ellclass{\function}{\measure}} = \int_\measurespace \function \, d\measure$.
Also by definition, 
$\expvalof{\condexpsub{\ellclass{\function}{\measure}}{\sigmaalgebraii}}
= \inducedint_\measurespace \condexpsub{\ellclass{\function}{\measure}}{\sigmaalgebraii}
\, d\measure.$
We will show that these two integrals are equal.
...
$\ellclass{\function}{\measure}\in L_1(\measurespace, \sigmaalgebra,\measure)$. This means that any equation involving conditional mathematical expectation which holds for all 
...
L_1(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$, 
so that 
$\ellclass{e}{\measure\restrictedto{\sigmaalgebraii}}=\kondexpsub{\ellclass{\functionii}{\measure}}{B}$,
and let the set 
$C\in\measurable{\Psubstar{\sigmaalgebraii}}
\subseteq
\measurable{\measure^\kernast}$.
We will show that 
$\int_B\charfunction{C} \functionii\, d\measure
=
\inducedint_B \ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}}\, d\measure\restrictedto{\sigmaalgebraii}$ for all $B\in\sigmaalgebraii$, which implies, by the uniqueness of conditional mathematical expectation of 
$\ellclass{\charfunction{C} \functionii}{\measure}$ with respect to $\sigmaalgebraii$, that 
$\kondexpsub{\ellclass{\charfunction{C} \functionii}{\measure}}{\sigmaalgebraii}
=
\ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}}$.
See that
\begin{align*}
\int_B\charfunction{C} \functionii\, d\measure
& =
...
\\
& =
\int_B \charfunction{C}e\, d\measure\restrictedto{\sigmaalgebraii}
\\
& =
\inducedint_B \ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}}\, d\measure\restrictedto{\sigmaalgebraii}\,.
\end{align*}
This shows that
\[
\kondexpsub{\ellclass{\charfunction{C} \functionii}{\measure}}{\sigmaalgebraii}
=
\ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}},\]
and since the equivalence class $\ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}}$
can be expressed as 
\begin{align*}
\ellclass{\charfunction{C}e}{\measure\restrictedto{\sigmaalgebraii}}
& =
\ellclass{\charfunction{C}}{\measure\restrictedto{\sigmaalgebraii}}\ellclass{e}{\measure\restrictedto{\sigmaalgebraii}}
\\
& =
...
it follows that 
\[
\kondexpsub{\bigr(\ellclass{\charfunction{C}}{\measure}\ellclass{\functionii}{\measure}\bigr)}
{\sigmaalgebraii}
=
\ellclass{\charfunction{C}}{\measure\restrictedto{\sigmaalgebraii}}\kondexpsub{\ellclass{\functionii}{\measure}}{\sigmaalgebraii}.
\]
...
%*********************************************************
%*********************************************************
\begin{defnboldnote}[Conditional probability]\label{conditional_probability}
Let
$(\measurespace, \sigmaalgebra, \measure)$
...
$\sigmaalgebraii$. 
It means given an $\mspaceelt\in\measurespace$ and a 
...
\]
The given measurable spaces
$(\measurespace, \sigmaalgebra)$ and
$(\measurespaceii, \sigmaalgebraii)$
need not be distinct, which may be apparent by the domain of
$\markovkernel$. 
...
The map
$\markovkernel\circ\fixinthesecond{\setii}$
may be denoted with something like
$\markovkernel^{\setii}$,
...
$(\measurespaceii, \sigmaalgebraii)$, possibly using the notation 
$\markovkernel:(\measurespace, \sigmaalgebra)\transition
(\measurespaceii, \sigmaalgebraii)$ in place of specifying
$\markovkernel:\measurespace\times\sigmaalgebraii\to [0,1]$.
\end{defn}
...
\begin{defnboldnote}[Regular conditional probability]\label{regular_version}
Let the collection
...
%*********************************************************
%*********************************************************
\begin{defnboldnote}[Conditional distribution]\label{conditional_distribution}
...
\term{conditionally independent \textsigma-subalgebras} given $\sigmaalgebraii$ 
if 
...
{$\condindep{\sigmaalgebraiii}{\sigmaalgebraiv}{\sigmaalgebraii}$,}
{the \textsigma-algebras $\sigmaalgebraiii$ and $\sigmaalgebraiv$ are conditionally independent given $\sigmaalgebraii$}
\end{defnboldnote}
...
\textsl{Principle of Real Analysis}~\cite{pora_aliprantis_1990}, Aliprantis and Burkinshaw.
\item
The conditional distribution material started with Loeve,
\textsl{Probability Theory}~\cite{pt_loeve_1960}.
\item
The Bayesian statistics here started as an elaboration of sections 1.2.1 and 1.2.2 in
