...
\begin{defn}
Proposition~\ref{equivalent_definition_regular_distribution} says that to each regular conditional distribution there corresponds a particular Markov kernel. We will call such a kernel the
...
$L_1(\uspace,\uspacesig,\measureiii)$, and can be written as
\begin{equation}
\condprobop{\sigalg{H}} \circ \preimage{(\funcf,\funcg)}
(\seti\times\setii)
=
...
\end{equation}
By the ``regularity'' hypothesis (spell this out better),
\[
...
is integrable with respect to $\measlambda$, it follows that
$\funck^\seti\cdot\funcl^\setii$
is integrable with respect to $\measlambda$.
According to the definition of the product of classes,
\begin{align*}
\ellclass{\funck^\seti}{}
\cdot
...
\end{align*}
By the regularity hypothesis, this says
\[
\condprobop{\sigalg{H}}\circ \preimage{\funcf}(\seti)
\cdot
\condprobop{\sigalg{H}}\circ \preimage{\funcg}(\setii)
...
\, d\measnu(\mspaceeltii)
\]
And by one of Fubini's or Tonelli's,
\[
\int_\seti p(\mspaceelt\mid\mspaceeltiii)
\, d\measmu(\mspaceelt)
\cdot
...
$\measurespace\times\measurespaceii$.
(But what is the qualifier on $\mspaceeltiii$? Likely $\marginalthree^\kernast$\hyp{}almost all
