...
\addchap{Preface}
\lettrine{T}{\,hese} notes were not initially intended for anyone else to look at.
...
The underlying reason these notes exist is so I could track the progress I had made while learning statistics.
I am putting these notes online, thinking  they may help another mathematician trying to learn statistics.


In these notes,
integration theory is based upon semi-rings as opposed to \textsigma\hyp{}algebras, to be explained further in the next few paragraphs.
Also,
\begin{itemize}
\item
...
In the statistics books that I had read, if the author tried to make clear which integral they were using, it was always the Lebesgue integral defined in terms of a measure on a  \textsigma\hyp{}algebra.
...
One main
difference:
In the case of a measure on a  \textsigma\hyp{}algebra and the consequent Lebesgue integral, say as developed in \textsl{Probability Theory,} Loeve~\cite{pt_loeve_1960}, the resulting conditional mathematical expectation is a class of functions, where each function in the class is measurable with respect to a
\textsigma\hyp{}subalgebra, but in the case of a measure on a semi-ring, as developed in
\textsl{Principles of Real Analysis,} Aliprantis \& Burkinshaw~\cite{pora_aliprantis_1990}, the consequent conditional mathematical expectation is a class of functions from which we can select a representative function which is measurable with respect to the  \textsigma\hyp{}subalgebra.  Not a big
difference,
but you still need to make the selection. Then again, you always need to make a selection with a regular conditional probability. Sorry about the pun.
...
%*********************************************************
%*********************************************************
\lettrine{T}{\,his} is an abbreviated review of measure theory, based upon semirings.
...
\begin{rmk}
An attempt at a categorical approach to measurable spaces and measurable functions which does not take into account collections of null sets would be considered a first attempt.  There is nothing wrong with first attempts. Who knows? You might end up factoring by an ideal.
\end{rmk}
...
\begin{rmk}
Underlying the mechanics of Proposition~\ref{measurable_almost_everywhere} and Theorem~\ref{measurable_limits} is the idea of what it means for a measure to be complete. This property describes a tight interplay between a measure and the collection of sets which forms its domain; let us say that a measure space is \emph{complete}\index{complete measure space} if every subset of a set of measure zero is measurable. It is almost worth ferreting this out, and then restating the proposition and theorem in these terms.   Vaguely, let $(\measurespace, \sigmaalgebra, \measure)$ be a complete measure space, where $\sigmaalgebra$ is at least an algebra of sets. Then if $f:\measurespace\to\R$ is an
...
$(\sigmaalgebra,\borel_\R)$\hyp{}measurable function. Since a measure space
$(\measurespace, \measurable{\measure^\kernast}, \measure^*)$ is always a complete measure space, the proposition would follow. But this is a type of interference that generates problems which I do not want to solve here.
\end{rmk}
...
Let $(\measurespace, \sigmaalgebra)$ be a measurable space, and let $f:\measurespace\to\R$, with $\funcf(x)\geq 0$ for all $x\in\measurespace$.
If $\funcf$ is an $(\sigmaalgebra, \borel_\R)$\hyp{}measurable function, then there is a sequence
$\{ \phi_n \}$ of $(\sigmaalgebra, \borel_\R)$\hyp{}simple functions such that
$0 \leq \phi_n(x) \uparrow \funcf(x)$ for all $x\in\measurespace$.
\end{prop}
...
Let $(\measurespace,\sigmaalgebra, \measmu)$ be a measure space, where $\sigmaalgebra$ is a
$\textsigma$\hyp{}algebra.
An
$(\sigmaalgebra, \borel_\R)$\hyp{}simple
function is an
$(\sigmaalgebra,\borel_\R)$\hyp{}\term{step function} if its preimages all have finite measure.  That is,  if $\phi$ is an
$(\sigmaalgebra, \borel_\R)$\hyp{}simple function
having range
$\{a_1, \ldots, a_n\}$, then
$\phi$ is an $(\sigmaalgebra,\borel_\R)$\hyp{}\term{step function} if
$\phi=\sum_{i=1}^n a_i\charfunction{\seti_i}$ with each
$\seti_i \in \sigmaalgebra$, and with each $\measmu(A_i) < \infty$,
where we can suppose the $A_i$ to be pairwise disjoint.
Is it clear that with a finite measure space,
every
$(\sigmaalgebra, \borel_\R)$\hyp{}simple
function is an
$(\sigmaalgebra, \borel_\R)$\hyp{}step function?
\end{defn}


It seems that one of the two following propositions should be redundant.  Replacing $\sigmaalgebra$ with
$\measmu^\kernast\mhyphen\mathfrak{Meas}$
does not quite do it.


\begin{prop}\label{sigma_finite_step_functions}
Let $(\measurespace, \semiring, \measure)$ be a $\textsigma$-finite measure space, and let
$f:\measurespace\to\R$
with $f(x)\geq 0$ for all $x\in\measurespace$.
If $\funcf$ is a \mbmeasurable\ function,
then there is a sequence $\{\phi_n\}$ of
$(\measurable{\measure^\kernast},\borel_\R)$\hyp{}step functions such that $\phi_n(x)\geq 0$ and $\phi_n(x)\uparrow f(x)$ for all $x\in\measurespace$.
\end{prop}




\begin{prop}\label{sigma_finite_a_measurable}
Let $(\measurespace, \sigmaalgebra, \measure)$ be a $\textsigma$-finite measure space, where $\sigmaalgebra$ is a \textsigma\hyp{}algebra, and let
$f:\measurespace\to\R$
with $f(x)\geq 0$ for all $x\in\measurespace$.
If $\funcf$ is an $(\sigmaalgebra,\borel_\R)$\hyp{}measurable function,
then there is a sequence $\{\phi_n\}$ of
$(\sigmaalgebra,\borel_\R)$\hyp{}step functions such that $\phi_n(x)\geq 0$ and $\phi_n(x)\uparrow f(x)$ for all $x\in\measurespace$.
\end{prop}
...
Let $\phi:\measurespace\to\R$ be a $(\measurable{\measure^\kernast},\borel_\R)$-step function such that $\phi=\sum_{i=1}^{n}a_i\charfunction{A_i},$ where the range of $\phi$ is the set $\{a_1, \ldots, a_n\}$, and each
$A_i \definedby \preimage{\phi}(\{ a_i\} )$
is $\measmu^\kernast$\hyp{}measurable.
Then the Lebesgue \emph{integral}\index{integral}  of $\phi$, denoted by $\int_\measurespace\phi\,d\measure$, or simply $\int \phi$,  is defined to be the real number
\[
...
\begin{discussion}\label{alternate_lebesgue_integral}
In some texts, the Lebesgue integral is defined relative to a measure space
$(\measurespace, \sigmaalgebra, \measure)$,
where $\sigmaalgebra$ is a \textsigma\hyp{}algebra.
Then, if
$\phi:\measurespace\to\R$ is an
$(\sigmaalgebra,\borel_\R)$-step function such that $\phi=\sum_{i=1}^{n}a_i\charfunction{A_i},$ where the range of $\phi$ is the set $\{a_1, \ldots, a_n\}$, and each
$A_i \definedby \preimage{\phi}(\{ a_i\} )$
is in $\sigmaalgebra$,
the Lebesgue integral is defined to be the real number
\[
\int_\measurespace\phi\,d\measure = \sum_{i=1}^{n}a_i\measmu(A_i).
\]
The resulting integral is sufficient to integrate a class of
$(\sigmaalgebra, \borel_\R)$\hyp{}measurable functions.
How does the integral in Definition~\ref{integral_step_function} compare with the integral defined here?
As a consequence of Proposition~\ref{sigma_finite_a_measurable}, if the measure space
$(\measurespace, \sigmaalgebra, \measmu)$ is \textsigma\hyp{}finite, then for
$(\sigmaalgebra, \borel_\R)$\hyp{}measurable functions, the two integrals agree.
\end{discussion}
...
\begin{prop}\label{upper_function_algebra}
If $\funcf$ and $\funcg$ are upper functions, then:
\begin{enumerate}
\item
The function
$\funcf + \funcg $ is an upper function with
$\int(\funcf + \funcg) \, d\measmu
=
\int \funcf \, d\measmu + \int \funcg \, d\measmu$.
\item
If\, $0 \leq a \in \R$, then
$a\funcf$ is an upper function and
$\int (a\funcf) \, d\measmu
=
a\int \funcf \, d\measmu$.
\item
The functions
$\funcf \vee \funcg$ and $\funcf \wedge \funcg$ are upper functions.
\end{enumerate}
\end{prop}


\begin{prop}
If $f$ and $g$ are upper functions with $f\leq g$ $\measure^\kernast$-almost everywhere, then
$\int f \leq \int g$.
\end{prop}


\begin{prop}\label{upper_monotone}
If $\funcf:\measurespace\to\R$ is a function with
$\{ \funcf_n \}$
a sequence of upper functions such that
$\funcf_n(x) \uparrow \funcf(x)$ for
$\measmu^\kernast$\hyp{}almost all $x\in\measurespace$, and if
$\lim \int \funcf_n \, d\measmu < \infty$, then
$\funcf$ is an upper function and
$\lim \int \funcf_n \, d\measmu = \int \funcf \, d\measmu$.
\end{prop}
...
and if $\functionii:\measurespace \to \R$ is a
$(\measurable{\measure^\kernast},\borel_\R)$\hyp{}measurable function with
$0\leq \functionii \leq \function$ $\measure^\kernast$\hyp{}almost everywhere, then
$\functionii$ is an integrable function.
\end{prop}
...
\begin{thmboldnote}[Levi]\label{levi}
Let $\{ \funcf_n \}$ be a sequence of functions integrable with respect to $\measmu$.
If $\funcf_n(x) \leq \funcf_{n+1}(x)$ for
$\measmu^\kernast$\hyp{}almost all $x\in\measurespace$,
and if\, $\lim \int \funcf_n \, d\measmu < \infty$,
then there is a function
$\funcf$ integrable with respect to $\measmu$ such that
$\funcf_n(x) \uparrow \funcf(x)$ for
$\measmu^\kernast$\hyp{}almost all
$x\in\measurespace$.  Consequently,
$\int \funcf_n\, d\measmu
\uparrow
\int \funcf \, d\measmu$.
\end{thmboldnote}


%\begin{thmboldnote}[The Lebesgue Dominated Convergence Theorem]\label{tldct}
%\end{thmboldnote}
...
\begin{rmk}
The $L_p(\measurespace, \semiring, \measure)$ spaces can be comparable; for example, let
$(\measurespace, \semiring, \measure)$ be a finite measure space, then the quotient space $L_2(\measurespace, \semiring, \measure)$ is a vector subspace of the quotient space
$L_1(\measurespace, \semiring, \measure)$. This is the heart of the correspondence theorem, stated in Proposition~\ref{correspondence_theorem},  which extends to vector spaces, and it would be necessary that the kernel
$\cali{N}_2(\measurespace, \semiring, \measure)$
be equal to
$\cali{N}_1(\measurespace, \semiring, \measure)$
...
\measurespace
\}$
with the measure $\measmu$ being Dirac measure $\diracmeasure{\mspaceelt}$ concentrated at a point $\mspaceelt\in\seti$, with $\seti$ non-empty, and with $\seti$ not equal to $\measurespace$.
Let
$\sigmaalgebraii = 
\{ 
\mtset,
...
$(\diracmeasure{x})^\kernast$\hyp{}measurable subsets of $\measurespace$ is
$\sigmaalgebra$,
and that the collection of
$({\diracmeasure{x}}\restrictedto{\sigmaalgebraii})^\kernast$\hyp{}measurable subsets of
$\measurespace$
is
$\sigmaalgebraii$.  The set $\seti$ is not
$({\diracmeasure{x}}\restrictedto{\sigmaalgebraii})^\kernast$\hyp{}measurable
since it fails to cut $\measurespace$ sharply with respect to
$({\diracmeasure{x}}\restrictedto{\sigmaalgebraii})^*$.

The only function in
$\cali{N}_1(\measurespace,
\sigmaalgebraii,
{\diracmeasure{x}}\restrictedto{\sigmaalgebraii}
)$
is the zero function since $\mtset$ is the only set of
$({\diracmeasure{x}}\restrictedto{\sigmaalgebraii})^\kernast$\hyp{}measure zero.

But the set
$\seti\comp$
is a set of $(\diracmeasure{x})^\kernast$-measure zero.
And since the function $\charfunction{\seti\comp}$ is
$\bigl(\measurable{(\diracmeasure{\mspaceelt})^\kernast},\borel_\R\bigr)$\hyp{}measurable,
and
$\charfunction{\seti\comp} = 0$ $(\diracmeasure{\mspaceelt})^\kernast$\hyp{}almost everywhere,
...
Conclude that
$\cali{N}_1(\measurespace, \sigmaalgebraii,
{\diracmeasure{x}}\restrictedto{\sigmaalgebraii})$
need not equal
$\cali{N}_1(\measurespace, \sigmaalgebra,
\diracmeasure{\mspaceelt})$.
\end{example}
...
\cali{L}_1(\measurespace, \sigmaalgebra, \measure)
\]
to suggest some kind of structure-preserving thing.
\end{quoting}
...
\begin{prop}\label{submeasurable_step_function}
Let $(\measurespace, \sigmaalgebra, \measmu)$ be a finite measure space, where
$\sigmaalgebra$ is a $\textsigma$\hyp{}algebra, and let $\sigmaalgebraii$ be a $\textsigma$\hyp{}subalgebra of
\,$\sigmaalgebra$.
If $\ellclass{f}{\measmu}
\in L_1(\measurespace, \sigmaalgebra, \measmu)$, and if $f$ is
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable, then there is a sequence
$\{ \phi_n \}$ of
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}step functions with
$\phi_n(x) \uparrow f(x)$ for all $x\in\measurespace$, where each $\phi_n$ is a sum of the form
\[
\sum_{i=1}^m b_i \charfunction{\setii_i}
\]
with each $\setii_i \in \sigmaalgebraii$.
\end{prop}

\begin{proof}
Let $f\in\cali{L}_1(\measurespace, \sigmaalgebra, \measure)$, and let $f$ be
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable. We can suppose that $f\geq 0$.
With $\funcf$ being $(\sigmaalgebraii, \borel_\R)$\hyp{}measurable and nonnegative, it follows by Proposition~\ref{measurable_implies_simple_function_limit}
that there is a sequence $\{\phi_n\}$ of $(\sigmaalgebraii, \borel_\R)$\hyp{}simple functions such that
$\phi_n(x)\geq 0$ and $\phi_n(x)\uparrow f(x)$ for all $x\in\measurespace$.
Each
$\phi_n$ is a sum of the form
\[
\sum_{i=1}^m b_i \charfunction{\setii_i}
\]
with each $\setii_i \in \sigmaalgebraii$.
By hypothesis,  $(\measurespace, \sigmaalgebra, \measmu)$
is a finite measure space which means that each
$\measmu^*(\setii_i)<\infty$.
And since $\sigmaalgebraii\subseteq\sigmaalgebra$ by hypothesis, and
$\sigmaalgebra \subseteq \measurable{\measmu^\kernast}$ by
Proposition~\ref{semiring_measurable}, it follows that each
$\setii_i \in\measurable{\measmu^\kernast}$, and so each $\phi_n$ is a
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}step function,
...
\begin{proof}
Let
$\ellclass{\funcf}{\measmu} \in L_1(\measurespace, \sigmaalgebra, \measure)$,
with $\funcf$ a
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable representative.
By Proposition~\ref{submeasurable_step_function}, there is a sequence
$\{ \phi_n \}$ of
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}step functions with
$\phi_n(x) \uparrow f(x)$ for all $x\in\measurespace$, where each $\phi_n$ is a sum of the form
\[
...
so $\{\phi_n\}$ is a sequence of $(\measurable{\measuresubstar{\sigmaalgebraii}},\borel_\R)$-step functions with
$\phi_n(x) \uparrow f(x)$ for all $x\in\measurespace$.
In order to complete the proof that $f$ is integrable with respect to
$\measmu\restrictedto{\sigmaalgebraii}$, it is sufficient to show that
$\lim \int_\measurespace \phi_n \, d \measure\restrictedto{\sigmaalgebraii} < \infty$.

Each $\phi_n$ is integrable with respect to $\measmu$ and with respect to
$\measmu\restrictedto{\sigmaalgebraii}$, and so by
Proposition ~\ref{integral_over_subalgebra},
\[
\int_\measurespace \phi_n \, d\measure
= \int_\measurespace \phi_n \, d\measure\restrictedto{\sigmaalgebraii}.
\]
By hypothesis, $\int \funcf \, d\measmu < \infty$, and
$\int_\measurespace f \, d\measure
=
\lim\int_\measurespace \phi_n \, d\measure$,
so
$\lim\int_\measurespace \phi_n \, d\measure <\infty$.
Then since 
$\int_\measurespace \phi_n \, d\measure
= 
\int_\measurespace \phi_n \, d\measure\restrictedto{\sigmaalgebraii}$,
...
$(\sigmaalgebra, \borel_\R)$\hyp{}measurable function is also a  \mbmeasurable\ function.  The converse need not hold.  That is, the function $f$ need \emph{not} also be $(\sigmaalgebra, \borel_\R)$\hyp{}measurable. Regardless, each class in
...
\begin{prop}\label{mixed_implies_miracle}
Let
$(\measurespace, \sigmaalgebra)$
and
$(\measurespaceii, \sigmaalgebraii)$
be measurable spaces,
and let $\measnu$
be a \textsigma-finite measure on $\sigmaalgebraii$.
Let $\measonprod$ be a finite measure on
$\productsig{\sigmaalgebra}{\sigmaalgebraii}$.
If\,
$\measonprod \abscont (\marginalone\times\measnu)^*$, then
...
=
\int_\setii
\dfrac{d\measonprod}{d(\marginalone\times\marginaltwo)^*}(\mspaceelt,\mspaceeltii)
\, d\marginaltwo(\mspaceeltii)
\]
...
\begin{proof}
This is just a corollary to the first half of Proposition~\ref{mixed_implies_marginal}
with a change of measure according to
$d\marginaltwo = (d\marginaltwo / d\measnu)\, d\measnu$.
\end{proof}
...
The assumption that the induced probability measure
$\measmu\circ \preimage{\function}$
is absolutely continuous with respect to Lebesgue measure $\measlambda$ is at least implied.
...
$\ellclass{\function}{\measure}$ is its integral
$\int_\measurespace\function\,d\mu$.
The operator $\expval$ is the linear functional defined by
\[
\expval:
...
$\measmu\circ\preimage{\funcf}$ of $\funcf$ is absolutely continuous with respect to a
\textsigma\hyp{}finite measure
...
where we suppose $\seti$, $\setii$, $\setiii$, and $\setiv$ to be $\measure^\kernast$-measurable sets of finite measure, with $\seti$ and $\setii$ disjoint, and $\setiii$ and $\setiv$ disjoint. Then
\begin{align*}
\function\functionii 
& =
...
\lettrine{C}{onditional} mathematical expectation is not difficult to define, but there seems to be some subtlety in the proof of its existence, especially when it comes to applying the Radon-Nikodym theorem.

We are supposing that the `$\sigmaalgebra$' in any probability space $(\measurespace, \sigmaalgebra,\measure)$ is a
$\textsigma$\hyp{}algebra, as opposed to merely a semiring.
The reason for supposing this is because the Radon-Nikodym theorem applies to measurable spaces, such as
$(\measurespace, \sigmaalgebra)$, in which case $\sigmaalgebra$ is certainly a \textsigma\hyp{}algebra.


\begin{defnboldnote}
\label{cme_sigma_algebra}
\index{conditional expectation with respect to a sigma algebra@conditional expectation with respect to a $\textsigma$\hyp{}algebra}
%Do \nomenclature[prefix]{symbol}{description}
%where prefix is used to order the symbol in the Nomenclature list.
...
Let $(\measurespace, \sigmaalgebra,\measure)$ be a probability space, and let
$\sigmaalgebraii$ be a \textsigma\hyp{}subalgebra of $\sigmaalgebra$.
For each class
$\lebclass{\funcf}$ in $L_1(\measurespace, \sigmaalgebra,\measure)$,
the \textbf{conditional mathematical expectation}
$\condexpsub{{\lebclass{\funcf}}}{\sigmaalgebraii}$
of $\lebclass{\funcf}$
with respect to
$\sigmaalgebraii$,
or simply the \emph{conditional expectation} of
...
%
\[
\int_\setii \funcf\, d\measure
= 
\int_\setii \funcg\, d\measure
\quad\text{for all $\setii \in \sigmaalgebraii$}.
\]
\end{defnboldnote}
...
$\lebclass{\funcf}$
in $L_1(\measurespace, \sigmaalgebra,\measure)$
have a $(\sigmaalgebraii,\borel_\R)$\hyp{}measurable representative,
say the function $\funcf$ itself is $(\sigmaalgebraii,\borel_\R)$\hyp{}measurable,
then
$\condexpsub{{\lebclass{\funcf}}}{\sigmaalgebraii}
=
\lebclass{\funcf}$
...
\quad\text{for all $\setii \in \sigmaalgebraii$}.
\]
And since by definition the class
$\condexpsub{{\lebclass{\funcf}}}{\sigmaalgebraii}$ does have a
$(\sigmaalgebraii,\borel_\R)$\hyp{}measurable representative function,
it follows that
$\condexpop{\sigmaalgebraii } (\condexpsub{ \lebclass{\funcf} }{ \sigmaalgebraii })
=
\condexpsub{ \lebclass{\funcf} }{ \sigmaalgebraii }.$
...
on $L_1 (\measurespace, \sigmaalgebra,\measure)$, or rather, the operator
$\condexpop{\sigmaalgebraii}$ is idempotent
...
%*********************************************************
%*********************************************************
\section{Properties of Expectation Operators}
%
...
\to
L_1(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii}):
\ellclass{\funcf}{\measmu}
\mapsto
\kondexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}
\]
and
\[
\condexpop{\sigmaalgebraii}:
...
\ellclass{\funcf}{\measmu}
\mapsto
\condexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}\]
are positive, linear, and contractive.
\end{prop}
...
\begin{rmk}
The correct and complete proof that
\[
\expval{\ellclass{\function}{\measure}}
= 
\expvalof{\condexpsub{\ellclass{\function}{\measure}}{\sigmaalgebraii}},
\]
as in the following proposition, took some effort, tracing back through at least Proposition~\ref{integral_over_subalgebra}.
...
%*********************************************************
%*********************************************************
\section{A Monotone Convergence Theorem}
%
\begin{rmk}
If $\ellclass{\funcf}{\measmu}$
is a class in
$L_1(\measurespace, \sigmaalgebra,\measmu)$,
then
$\ellclass{0}{\measmu} \leq \ellclass{\funcf}{\measmu}$
means that
$0 \leq \funcf(\mspaceelt)$ for
$\mu^\kernast$\hyp{}almost
all
$\mspaceelt \in \measurespace$.
Equivalently,
$\ellclass{\funcf}{\measmu} \leq \ellclass{\funcg}{\measmu}$
means that
$\funcf(\mspaceelt) \leq \funcg(\mspaceelt)$ for
$\mu^\kernast$\hyp{}almost
all
$\mspaceelt \in \measurespace$.
What then does
$\ellclass{\funcf_n}{\measmu}
\uparrow
\ellclass{\funcf}{\measmu}$
mean? It means that for each $n$,
$\funcf_n(\mspaceelt) \leq \funcf_{n+1}(\mspaceelt)$ for
$\mu^\kernast$\hyp{}almost
all
$\mspaceelt \in \measurespace$,
and that
$\funcf_n(\mspaceelt) \to \funcf(\mspaceelt)$ for
$\mu^\kernast$\hyp{}almost
all
$\mspaceelt \in \measurespace$.

Far better would have been to simply say,
``Remember that
$L_1(\measurespace, \sigmaalgebra,\measmu)$
is a Banach lattice.''
\end{rmk}
...
\begin{prop}\label{conditional_monotone}
Let $(\measurespace, \sigmaalgebra,\measmu)$ be a probability space, and let
$\sigmaalgebraii$ be a \textsigma\hyp{}subalgebra of $\sigmaalgebra$.
Let
$\{
\ellclass{\funcf}{\measmu},
\ellclass{\funcf_1}{\measmu},
\ellclass{\funcf_2}{\measmu},
\ldots \}$
be a collection of classes in
$L_1(\measurespace, \sigmaalgebra, \measmu)$.
If\,
$\ellclass{0}{\measmu}
\leq
...
\ellclass{\funcf}{\measmu}$,
then
$\ellclass{0}{\measmu\restrictedto{\sigmaalgebraii}}
\leq
\kondexpop_\sigmaalgebraii \ellclass{\funcf_n}{\measmu}
\uparrow
\kondexpop_\sigmaalgebraii \ellclass{\funcf}{\measmu}$.
\end{prop}

\begin{proof}
Fairly simple consequence of Levi's Theorem~\ref{levi} applied to
$\kondexpop_\sigmaalgebraii \ellclass{\funcf_n}{\measmu}$,
and of the definition of conditional mathematical expectation.
\end{proof}
...
%*********************************************************
%*********************************************************
\section{Atoms and Independence}
%
...
%*********************************************************
%*********************************************************
\section{Measurable Functions Factor Out}
%
...
\textbf{The condition
$\ellclass{\function}{\measmu\restrictedto{\sigmaalgebraii}}
\cdot
\kondexpsub{\ellclass{\funcg}{\measmu}}{\sigmaalgebraii}$
is in
$L_1(\measurespace, \sigmaalgebraii, \measmu\restrictedto{\sigmaalgebraii})$
is extraneous.}

\begin{prop} \label{measurable_functions_factor_out}
Let $(\measurespace, \sigmaalgebra,\measmu)$ be a probability space.
Let $\sigmaalgebraii$ be a $\textsigma$-subalgebra of $\sigmaalgebra$,
and let the classes
$\ellclass{\funcf}{\measmu}$
and
$\ellclass{\funcg}{\measmu}$ be in
$L_1(\measurespace, \sigmaalgebra,\measmu)$.
If the function $\function:\measurespace\to\R$ is
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable,  and if the class
$\ellclass{\funcf}{\measmu} \cdot \ellclass{\funcg}{\measmu}$
is in
$L_1(\measurespace, \sigmaalgebra,\measmu)$,
then
$\ellclass{\function}{\measure\restrictedto{\sigmaalgebraii}}
...
\begin{proof}
Let the function $\function:\measurespace\to\R$ be
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable,  and let the class
$\ellclass{\funcf}{\measmu} \cdot \ellclass{\funcg}{\measmu}$,
which equals
$\ellclass{\funcf\funcg}{\measmu}$ by definition,
be in
$L_1(\measurespace, \sigmaalgebra,\measmu)$.

If we already knew that the product of classes
$\ellclass{\function}{\measure\restrictedto{\sigmaalgebraii}}
\cdot
\kondexpsub{\ellclass{\functionii}{\measure}}{\sigmaalgebraii}$
existed in
$L_1(\measurespace, \sigmaalgebraii, \measmu\restrictedto{\sigmaalgebraii})$,
then the proof would be foolishly simple.  But it is not.

Assuming momentarily that the symbols
$\ellclass{\function}{\measure\restrictedto{\sigmaalgebraii}}
\cdot
\kondexpsub{\ellclass{\functionii}{\measure}}{\sigmaalgebraii}$
make sense, then what would it mean to show that
\[
\kondexpsub{\bigl(\ellclass{\function}{\measure}
\cdot
\ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
=
\ellclass{\function}{\measure\restrictedto{\sigmaalgebraii}}
\cdot
\kondexpsub{\ellclass{\functionii}{\measure}}{\sigmaalgebraii}
...
is the unique class in
$L_1(\measurespace, \sigmaalgebraii, \measmu\restrictedto{\sigmaalgebraii})$
having a $( \sigmaalgebraii, \borel_R )$\hyp{}measurable representative such that
\[
\int_\setii \funcf\funcg \,d\measmu
=
\inducedint_\setii
\kondexpsub{\bigl(\ellclass{\function}{\measure}
\cdot
\ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
\, d \measmu\restrictedto{\sigmaalgebraii}
\]
for all $\setii\in\sigmaalgebraii$.
...
\]
for all $\setii\in\sigmaalgebraii$.
And this is equivalent to showing, letting
$e \in
\kondexpsub{\ellclass{\funcg}{\measmu}}{\sigalg{B}}$,
that
\[
\int_\setii \funcf\funcg \,d\measmu
=
...
\]
for all $\setii\in\sigmaalgebraii$.
We will show that this equality holds when the function $\funcf$ is replaced by suitable step functions, and then take limits.

By hypothesis, the class
$\ellclass{\funcf}{\measmu}$
is in
$L_1(\measurespace, \sigmaalgebra,\measmu)$,
and the function
...
so by Proposition~\ref{submeasurable_step_function}, there is a sequence
$\{ \phi_n \}$ of
$(\measurable{\measmu^\kernast}, \borel_\R)$\hyp{}step functions with
$\phi_n(x) \uparrow f(x)$ for all $x\in\measurespace$, where each $\phi_n$ is a finite sum of the form
\[
\phi_n = \sum_i c_i \charfunction{\setiii_i}
\]
with each $\setiii_i \in \sigmaalgebraii$.


We can suppose that both $\funcf$ and $\funcg$ are nonnegative.
It means we can further suppose that the $\phi_n$ are nonnegative.
It also means the $\phi_n g$ and $\funcf \funcg$ are upper functions.
Since
$\phi_n \funcg \uparrow \funcf \funcg$,
and since $\funcf\funcg$ is integrable,
it follows by Proposition~\ref{upper_monotone}
that
\[
\lim \int_\setii \phi_n \funcg\, d\measmu
=
\int_\setii \lim \phi_n\funcg \, d\measmu
=
\int_\setii \funcf \funcg \, d\measmu .
\]
Similarly, \textbf{Oops, have to SHOW $fe$ is int'l.}
\[
\lim \int_\setii \phi_n e \, d\measmu\restrictedto{\sigmaalgebraii}
=
\int_\setii \lim \phi_n e \, d\measmu\restrictedto{\sigmaalgebraii}
=
\int_\setii \funcf e \, d\measmu\restrictedto{\sigmaalgebraii} .
\]
To complete the proof then, it is sufficient to show that
\[
\int_\setii \phi_n \funcg \, d\measmu
=
\int_\setii \phi_n e \, d\measmu\restrictedto{\sigmaalgebraii},
\]
and since each
$\phi_n = \sum_{i=1}^m c_i \charfunction{\setiii_i}$ with
$\setiii_i\in\sigmaalgebraii$,
in turn it is sufficient to show that
\[
\int_\setii \charfunction{\setiii} \funcg \, d\measmu
=
\int_\setii \charfunction{\setiii} e \, d\measmu\restrictedto{\sigmaalgebraii},
\]
for all $\setiii\in\sigmaalgebraii$.
...
\]
for all $\setii\in\sigmaalgebraii$,
and if $\setiii\in\sigmaalgebraii$, then $\setii\meets\setiii\in\sigmaalgebraii$, so
\[
\int_{\setii\meets\setiii} \funcg \, d\measmu
=
\int_{\setii\meets\setiii}
e
\, d\measmu\restrictedto{\sigmaalgebraii}.
\]
If $\setiii\in\sigmaalgebraii$, then $\setiii$ is
$\measurable{\measmu^\kernast}$, and so
\[
\int_{\setii\meets\setiii} \funcg \, d\measmu
=
\int_\setii \charfunction{\setiii} \funcg \, d\measmu ,
\]
and $\setiii$ is also
$\measurable{\measmu\restrictedto{\sigmaalgebraii}^\kernast}$, and similarly
\[
\int_{\setii\meets\setiii}
e
\, d\measmu\restrictedto{\sigmaalgebraii}
...
\condexpsub{\ellclass{\functionii}{\measure}}{\sigmaalgebraii}
\]
is a consequence, recall that we are letting
$e \in
\kondexpsub{\ellclass{\funcg}{\measmu}}{\sigalg{B}}$.
We have shown that
\[
...
\ellclass{\function}{\measure\restrictedto{\sigmaalgebraii}}
\cdot
\ellclass{e}{\measmu\restrictedto\sigmaalgebraii} \\
& =
\ellclass{\funcf e }{\measmu\restrictedto\sigmaalgebraii}
\end{align*}
saying that
\[
\kondexpsub{\bigl(\ellclass{\function}{\measure} \cdot \ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
...
\ellclass{\funcf e }{\measmu\restrictedto\sigmaalgebraii}.
\]
Under the embedding $\L1embedding$,
\[
\kondexpsub{\bigl(\ellclass{\function}{\measure} \cdot \ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
\mapsto
\condexpsub{\bigl(\ellclass{\function}{\measure} \cdot \ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
...
\ellclass{\funcf e }{\measmu\restrictedto\sigmaalgebraii}
\mapsto
\ellclass{\funcf e}{\measmu} =
\ellclass{\funcf}{\measmu} \cdot \ellclass{e}{\measmu},
\]
so
\[
\condexpsub{\bigl(\ellclass{\function}{\measure} \cdot \ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
...
=
\ellclass{e }{\measmu\restrictedto\sigmaalgebraii}$,
so under the embedding
$\condexpsub{(\ellclass{\functionii}{\measure})}{\sigmaalgebraii}
=
\ellclass{e }{\measmu}$, and by substitution
\[
\condexpsub{\bigl(\ellclass{\function}{\measure} \cdot \ellclass{\functionii}{\measure}\bigr)}{\sigmaalgebraii}
=
\ellclass{\funcf}{\measmu} \cdot \condexpsub{(\ellclass{\functionii}{\measure})}{\sigmaalgebraii},
\]
as required.
\end{proof}
...
{\scriptsize{$\markovkernel_\mspaceelt$
...
We will say that any map
$\measurespace\times\sigmaalgebraii\to \R$
or
$\measurespace\times\sigmaalgebraii\to [0,1]$
\term{defines a Markov kernel}
if it its range is $[0,1]$ except for possibly those $\mspaceelt$ in some null subset of
$\measurespace$ and if it can be redefined on that null subset, together with a possible redefinition of the codomain, in such a way that it satisfies the above definition.
...
\begin{prop}\label{marginal_implies_markov}
If\,
$\measonprod \abscont (\marginalone\times\marginaltwo)^*$
on
$\productsig{\sigmaalgebra}{\sigmaalgebraii}$,
...
$(\measurespaceii,\sigmaalgebraii)\transition (\measurespace, \sigmaalgebra)$,
and symmetrically, the map
\[
\measurespace\times\sigmaalgebra\to \R
:(\mspaceelt,\setii)\mapsto
\int_\setii
\dfrac{d\,\measonprod}{d(\marginalone\times\marginaltwo)^*}
(\mspaceelt,\mspaceeltii)\, d\marginaltwo(\mspaceeltii)
\]
defines a Markov kernel
$(\measurespace,\sigmaalgebra)\transition (\measurespaceii, \sigmaalgebraii)$.
\end{prop}


\begin{proof}
We will only show the first part, namely that we get a Markov kernel
$(\measurespaceii,\sigmaalgebraii)\transition (\measurespace,\sigmaalgebra)$.
By hypothesis,
...
\]
At this stage, we can only say that the codomain of $\funcj$ is $\R$.
We will show this map can be redefined on a subset of $\measurespaceii$ of
$\marginaltwo^\kernast$\hyp{}measure zero, resulting in a  Markov kernel.
...
\begin{prop}\label{markov_composition_measurable}
If
$\funcj: \measurespaceii \times \sigmaalgebra \to [0,1]$
is a Markov kernel
$(\measurespaceii,\sigmaalgebraii) \transition (\measurespace, \sigmaalgebra)$,
and if $\funcg : (\uspace, \sigalg{G}) \to (\measurespaceii, \sigmaalgebraii)$ is measurable, then
\[
\funck:\uspace\times\sigmaalgebra \to [0,1]
:(\uspaceelt,\seti) \mapsto \funcj \bigl( \funcg(\uspaceelt), \seti \bigr)
\]
is a Markov kernel
$(\uspace,\sigalg{G}) \transition (\measurespace, \sigmaalgebra)$.
\end{prop}
...
By Proposition~\ref{marginal_implies_markov}, the map
\[
\funcj
:\measurespaceii\times\sigmaalgebra\to \R
:(\mspaceeltii,\seti)\mapsto
...
$\funcg:\uspace\to\measurespace$ is $(\uspacesig,\sigmaalgebraii)$\hyp{}measurable, and therefore $(\sigalg{G}, \sigmaalgebraii)$\hyp{}measurable since
$\sigalg{G} = \preimage{\funcg}(\sigmaalgebraii)$. By Proposition~\ref{markov_composition_measurable}, the map
\[
\funck:\uspace\times\sigmaalgebra \to [0,1]
:(\uspaceelt,\seti) \mapsto \funcj \bigl( \funcg(\uspaceelt), \seti \bigr)
\]
...
\[
\preimage{\projectionone}
:\pspacesig\to\productsig{\pspacesig}{\sspacesig}
:\seti \mapsto \seti\times\measurespaceii
\]
and
\[
\preimage{\projectiontwo}
:\sspacesig\to\productsig{\pspacesig}{\sspacesig}
:\setii \mapsto \measurespace\times\setii,
\]
as illustrated:
\[
\xymatrix@R=16pt@C=30pt{
...
$\condprobop{\preimage{\projectionone}(\sigmaalgebra)}
\circ\,
\preimage{\projectiontwo}$ of
$\projectiontwo$
on $\sspacesig$
given $\preimage{\projectionone}(\sigmaalgebra)$ the \term{sampling
...
\productsig{\sigmaalgebra}{\sigmaalgebraii},
\measonprod)
\ar[r]^-{\condexpop{\preimage{\projectionone}(\sigmaalgebra)}}
& L_1(\measurespace\times\measurespaceii,
\productsig{\sigmaalgebra}{\sigmaalgebraii},
\measonprod)
\\
\productsig{\sigmaalgebra}{\sigmaalgebraii}
\ar[u]^-{Q\, \circ\, \charfunction{}}
\ar[ur]_{\condprobop{\preimage{\projectionone}(\sigmaalgebra)}}
& \sspacesig
\ar[l]^-{\preimage{\projectiontwo}}
\ar[u]_{\condprobop{\preimage{\projectionone}(\sigmaalgebra)}\circ\,\preimage{\projectiontwo}}
}% end \xymatrix
\]


Call the  conditional distribution
$\condprobop{\preimage{\projectiontwo}(\sigmaalgebraii)}
\circ\,
\preimage{\projectionone}$ of
$\projectionone$
on $\pspacesig$
given $\preimage{\projectiontwo}(\sigmaalgebraii)$ the \term{posterior conditional distribution}, as illustrated:
\[
\xymatrix@R=40pt{
L_1(\measurespace\times\measurespaceii,
\productsig{\sigmaalgebra}{\sigmaalgebraii},
\measonprod)
\ar[r]^-{\condexpop{\preimage{\projectiontwo}(\sigmaalgebraii)}}
& L_1(\measurespace\times\measurespaceii,
\productsig{\sigmaalgebra}{\sigmaalgebraii},
\measonprod)
\\
\productsig{\sigmaalgebra}{\sigmaalgebraii}
\ar[u]^-{Q\, \circ\, \charfunction{}} \ar[ur]_{\condprobop{\preimage{\projectiontwo}(\sigmaalgebraii)}}
& \pspacesig
\ar[l]^-{\preimage{\projectionone}}
\ar[u]_{\condprobop{\preimage{\projectiontwo}(\sigmaalgebraii)} \circ\, \preimage{\projectionone}}
}% end \xymatrix
\]
...
$\measonprod\abscont (\marginalone\times\marginaltwo)^*$.
Then both the sampling conditional distribution and the posterior conditional distribution are regular, which is the heart of Proposition~\ref{marginal_implies_regular}.
...
$\sigmaalgebraii$, called the \term{sampling probabilities},
...
a given \emph{prior} probability measure $\wouldbeprior$ on the parameter space \textsigma-algebra
$\pspacesig$ by
\[
...
\]
Then, by Proposition~\ref{not_a_product_measure}, the given \emph{prior} probability measure
$\wouldbeprior$ must equal the marginal, or \emph{prior}, probability distribution
$\prior$.
...
$\funcf_\pspaceelt:
\measurespaceii\to\R$
is equal to the Radon-Nikodym derivative:
...
and so by Proposition~\ref{mixed_implies_marginal},
...
by Proposition~\ref{mixed_implies_marginal}.
...
\quad \text{for all $\sspaceset\in\sspacesig$},
\]
by Proposition~\ref{mixed_implies_miracle},
and the family $\{ \prior_\sspaceelt :\sspaceelt\in\sspace \}$ of probability measures on the parameter space \textsigma-algebra $\pspacesig$,
called the \term{posterior probabilities},
defined for those
$\sspaceelt\in\sspace$ such that $\funcf^\sspaceelt$ is integrable with respect to
$\prior$, is defined by
...
%*********************************************************
%*********************************************************
\lettrine{W}{e barely} touch on a few points concerning conditional independence.
...
It follows by Remark~\ref{idempotent} that the condition
\[
\condexpsub{\ellclass{\charfunction{\setiii\meets\setiv}}{\measure}}{\sigmaalgebraii}
=
\condexpsub{\ellclass{\charfunction{\setiii}}{\measure}}{\sigmaalgebraii}
...
\newcommand{\helt}{x}
\newcommand{\heltii}{y}
\newcommand{\hortho}{e_1}
\newcommand{\horthoii}{e_2}
\newcommand{\horthoi}{e_i}
\newcommand{\horthon}{e_n}
...
Let $\hilbertspace$ be a Hilbert space, and let $\hilbertsubspace$ be a closed subspace of $\hilbertspace$. To each $\helt\in\hilbertspace$ there corresponds a unique element of $\hilbertsubspace$, denoted here by
$\proj(\helt)$,  such that
$\helt-\proj(\helt) \perp \hilbertsubspace$.
The map $\hilbertspace \to \hilbertspace$ defined by
$\helt\mapsto \proj(\helt)$ is called the orthogonal projection of
$\hilbertspace$ onto $\hilbertsubspace$ along ${\hilbertsubspace}^\perp$, or simply the
\emph{orthogonal projection of $\hilbertspace$
onto $\hilbertsubspace$}.
...
For $\helt$ and $\heltii$ in $\hilbertspace$, we let
$\inner{\helt}{\heltii}$ denote their inner product.

\begin{prop}
Let $\hilbertspace$ be a Hilbert space, and let $\hilbertsubspace$ be a closed subspace of $\hilbertspace$, where $\proj :\hilbertspace \to \hilbertspace$ is the orthogonal projection of $\hilbertspace$ onto $\hilbertsubspace$. Fix $\helt\in \hilbertspace$. Then $\proj(\helt)$ is that element of $\hilbertsubspace$ which is closest to $\helt$, meaning that
\[
\norm{\helt-\proj(\helt)}<\norm{\helt-\heltii}
\]
for all $\heltii$ in the subspace $\hilbertsubspace$ such that
$\proj(\helt)\neq \heltii$.
\end{prop}
...
\begin{prop}
Let $\{\hortho, \horthoii, \ldots, \horthon\}$ be an orthonormal set in a Hilbert space $\hilbertspace$, and let $\proj:\hilbertspace \to \hilbertspace$ be the orthogonal projection of $\hilbertspace$ onto the closed subspace
$\raum{Span}\{\hortho, \horthoii, \ldots, \horthon\}$.
Fix $\helt\in\hilbertspace$. Then
$\proj(\helt) = \sum_{i=1}^{n} \inner{\helt}{\horthoi}\horthoi$.
That is,
\[
\norm{\helt - \sum_{i=1}^n a_i \horthoi} <
\norm{\helt-\heltii}
\]
for all
$\heltii$ in the $\raum{Span}\{\hortho, \horthoii, \ldots, \horthon\}$
such that $\proj(\helt)\neq \heltii$
if and only if  each scalar  $a_i$ equals the Fourier coefficient
$\inner{\helt}{\horthoi}$ of $\helt$.
\end{prop}
...
\begin{rmk}
Some implications may be used without much fanfare; namely, if the classes
$\ellclass{\funcf}{\measmu}$ and
$\ellclass{\funcg}{\measmu}$ are in
$L_2(\measurespace, \sigmaalgebra,\measmu)$,
then the product of classes
$\ellclass{\funcf}{\measmu}
\cdot
\ellclass{\funcg}{\measmu}
\definedby
\ellclass{\funcf \funcg}{\measmu}$
is in
$L_1(\measurespace, \sigmaalgebra, \measmu)$.
This is a routine application of the Cauchy-Schwarz inequality, considered to be the
$2+2$
...
Let $(\measurespace, \sigmaalgebra, \measmu)$
...
restricted to
$L_2(\measurespace, \sigmaalgebra,\measmu)$
is  the unique orthogonal projection of the Hilbert space
$L_2(\measurespace, \sigmaalgebra,\measmu)$
onto the closed subspace
$\L1embedding
\bigl(
L_2(\measurespace, \sigmaalgebraii,\measmu\restrictedto{\sigmaalgebraii})
\bigr)
$.
\end{prop}
...
\begin{proof}
Let $\hilbertsubspace$ denote the subspace
$\L1embedding\bigl(
L_2(\measurespace, \sigmaalgebraii,\measmu\restrictedto{\sigmaalgebraii})
\bigr)$.
The elements of $\hilbertsubspace$ are the classes of
$L_2(\measurespace, \sigmaalgebra, \measmu)$
which have a
$(\sigmaalgebraii,\borel_\R)$\hyp{}measurable representative.

We will show that to each
$\ellclass{\funcf}{\measmu}
\in
L_2(\measurespace, \sigmaalgebra,\measmu)$
there corresponds a unique element of
$\hilbertsubspace$,
namely
$\condexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}$,
such that
...
\condexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}
\perp
\ellclass{\funch}{\measmu}$ for all
$\ellclass{\funch}{\measmu}
\in
\hilbertsubspace$,
for then the map
$\ellclass{\funcf}{\measmu}
...
$L_2(\measurespace, \sigmaalgebra,\measmu)$
onto $\hilbertsubspace$, per the first paragraph of this chapter.
...
\[
\inner{\ellclass{\funcf}{\measmu}}{\ellclass{\funch}{\measmu}}
=
\inner{\condexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}}
{\ellclass{\funch}{\measmu}}
,
\]
and in turn, this means showing that
\[
\expvalof{\ellclass{\funcf}{\measmu}
\cdot
\ellclass{\funch}{\measmu}}
=
\expval\bigl(
...
\textbf{Stop work here.
Go back and finish Proposition~\ref{measurable_functions_factor_out} first.}

We will apply Proposition~\ref{measurable_functions_factor_out}, and in order to do this, check first that
$\ellclass{\funcf}{\measmu} \in L_1(\measurespace, \sigmaalgebra,\measure)$, and this is because
$\ellclass{\funcf}{\measmu} \in L_2(\measurespace, \sigmaalgebra,\measure)$, and because
...
$(\sigmaalgebraii, \borel_\R)$\hyp{}measurable
since
$\ellclass{\funch}{\measmu}\in L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$.
Finally check that
$\ellclass{\funcf}{\measmu} \cdot \ellclass{\funch}{\measmu}
\in
L_1(\measurespace, \sigmaalgebra,\measure)$ by Cauchy-Schwarz.
Therefore,
since
$\ellclass{\funcf}{\measmu} \in L_1(\measurespace, \sigmaalgebra,\measure)$ and
...
$\expvalof{\ellclass{\funcf}{\measmu} \cdot \ellclass{\funch}{\measmu}}
=
\expvalof{\ellclass{\funch}{\measmu}
\cdot
\condexpsub{\ellclass{\funcf}{\measmu}}{\sigmaalgebraii}}$, as required.
...
such that
$\function - \condexpsub{\function}{\sigmaalgebraii} \perp \ellclass{\funch}{\measmu}$ for all $\ellclass{\funch}{\measmu}\in
L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$.
Let
...
$\function-\functionii\perp \ellclass{\funch}{\measmu}$ for all $\ellclass{\funch}{\measmu}\in
L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$.
We want to see that this implies $\functionii$ must equal
$\condexpsub{\function}{\sigmaalgebraii}$.
To say that
\[
\function-\functionii\perp \ellclass{\funch}{\measmu}
\quad\text{for all $\ellclass{\funch}{\measmu}\in
L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$}
\]
means saying that
\[
\inner{\function-\functionii}{\ellclass{\funch}{\measmu}} = 0
\quad\text{for all
$\ellclass{\funch}{\measmu}\in L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$},
\]
or equivalently that
\[
\inner{\function}{\ellclass{\funch}{\measmu}} = \inner{\functionii}{\ellclass{\funch}{\measmu}}
\quad\text{for all
$\ellclass{\funch}{\measmu}\in L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$}.
\]
In terms of integrals, this says
\[
\int_\measurespace \function\ellclass{\funch}{\measmu} \, d\measure
=
\int_\measurespace \functionii\ellclass{\funch}{\measmu} \, d\measure
\quad\text{
for all
$\ellclass{\funch}{\measmu}\in L_2(\measurespace, \sigmaalgebraii,\measure\restrictedto{\sigmaalgebraii})$}.
\]
...
See Discussion~\ref{the_cheapest_subalgebra}.
\end{example}
