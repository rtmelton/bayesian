...
\textbf{The hypothesis $\measonprod \abscont (\marginalone\times\measnu)^*$ in the following proposition implies that the conditional distribution
$\condprobop{\sigalg{G}}\circ \preimage{\funcf}$ of $\funcf$ on
$\sigmaalgebra$ given $\sigalg{G}$ is regular.
You want to re\hyp{}write the statement of the theorem, and in turn modify the proof.}
...
\text{for $h(u) = z$.}
\]
Since this Radon-Nikodym derivative has been described as representing
``the conditional distribution of $f$ given $h=z$,'' this might add credence to the `$\vert$' symbol in expressions like $p(x\mid z)$, or
$\measlambda_{\funcf\vert \funch = \mspaceeltiii}$.
\end{notn}
...
\]
The projection $\projectionone$
is $\bigl(\productsig{\pspacesig}{\sspacesig},\pspacesig \bigr)$\hyp{}measurable and
...
The projection $\projectiontwo$
is $\bigl(\productsig{\pspacesig}{\sspacesig},\sspacesig\bigr)$\hyp{}measurable and
induces the marginal probability distribution
