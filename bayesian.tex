...
Also, set the joint $\measonprod = \measureiii\circ\preimage{(\function,\functioniii)}$,
...
with respect to the joint $\measonprod$,
then 
\[
...
measurable, and set
$\sigalg{H} = \preimage{\functioniii}(\sigmaalgebraiii)$.
Let $\measmu$ and $\measrho$ be \textsigma\hyp{}finite measures on respective \textsigma\hyp{}algebras $\sigmaalgebra$ and $\sigmaalgebraiii$.
Also, set the joint $\measonprod = \measureiii\circ\preimage{(\function,\functioniii)}$,
and set the marginal $\marginalthree = \measlambda\circ\preimage{\funch}$.
If the joint
$\measonprod\abscont (\measmu\times\measrho)^*$, with
$\funck$
denoting the corresponding Markov kernel given
$(\uspace,\sigalg{H})$ and $(\measurespace,\sigmaalgebra)$,
then the measure
$\funck_\uspaceelt$
on $\sigmaalgebra$, where $\uspaceelt$ is such that $\funch(\uspaceelt)=\mspaceeltiii$,
...
\begin{proof}
Let
$\measonprod\abscont (\measmu\times\measrho)^*$.
By Proposition~\ref{joint_abscont} and Proposition~\ref{absolute_continuity_regular}, the conditional distribution
\[
\condprobop{\sigalg{H}}\circ\, \preimage{\function}:
\sigmaalgebra \to L_1(\uspace,\uspacesig,\measureiii)
\]
of $\function$ on $\sigmaalgebra$ given $\sigalg{H}$
is regular. We are letting
$\funck$
denote the corresponding Markov kernel given
$(\uspace,\sigalg{H})$ and $(\measurespace,\sigmaalgebra)$, as illustrated:
\[
...
\]
By Proposition~\ref{marginal_abscont}, the marginal
$\marginalthree \abscont \measrho$.
It is sufficient to show that if $\seti\in\sigmaalgebra$, then
\[
\funck_\uspaceelt(\seti)
=
\int_\seti 
...
\,d\measure(\mspaceelt),
\]
where $\mspaceeltiii=\funch(\uspaceelt)$.

We will start by showing that if a function
$\funcj:\measurespace\times\measurespaceiii \to \R$ is integrable over
$\measurespace\times\measurespaceiii$
with respect to the joint $\measonprod$, then
\[
\int_{\measurespace\times\measurespaceiii}
\funcj \,d\measonprod
= 
...
Let $\funcj$ be integrable over
$\measurespace\times\measurespaceiii$
with respect to $\measonprod$.
By Proposition~\ref{radon_nikodym_corollary},
\[
\int_{\measurespace\times\measurespaceiii}
\funcj \,d\measonprod
=
...
$\funcj \cdot
\dfrac{d\, \joint}
{d(\measure\times\measureiv)^*}$ is integrable with respect to
$\measure\times\measureiv$, we can apply Fubini's Theorem~\ref{fubini}:
\[
\int_{\measurespace\times\measurespaceiii}
\funcj \cdot
\dfrac{d\, \joint}
...
\section{Colloquial Density Notation}
